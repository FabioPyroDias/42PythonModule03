...
def distance_between_points(point_start: tuple, point_end: tuple) -> float:
    return math.sqrt((point_end[0] - point_start[0])
                     ** 2 + (point_end[1] - point_start[1])
                     ** 2 + (point_end[2] - point_start[2]) ** 2)
...
if __name__ == "__main__":
    ...
    print(f"Distance between ({origin[0]}, {origin[1]}, {origin[2]}) and "
          f"({pos0[0]}, {pos0[1]}, {pos0[2]}): {distance0:.2f}")
    print()
    print("Parsing coordinates: \"3,4,0\"")
    pos1 = parse_position("3,4,0")
    distance1 = distance_between_points(origin, pos1)
    print(f"Distance between ({origin[0]}, {origin[1]}, {origin[2]}) and "
          f"({pos1[0]}, {pos1[1]}, {pos1[2]}): {distance1:.1f}")
    print()
